...
Deform_Index:	dc.w Deform_GHZ-Deform_Index, Deform_LZ-Deform_Index
		...
		dc.w Deform_GHZ-Deform_Index, Deform_Secret-Deform_Index
; ---------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------
; Secret Zone background layer deformation code
; ---------------------------------------------------------------------------
...
Deform_Secret:
	; plain background scroll
		...
		move.b	(secretBoss).w,d0
		bmi.w	Deform_SecretBoss
		cmpi.b	#1,d0
		beq.w	Deform_SecretBoss

		lea	Sine_Data,a0
		move.b	v_lz_deform.w,d0
		andi.w	#$FF,d0
		add.w	d0,d0
		adda.w	d0,a0
		
		addq.b	#1,v_lz_deform.w
		
		lea	(v_hscrolltablebuffer).w,a1
		move.w	(v_screenposx).w,d0
		neg.w	d0
		move.w	(v_bgscreenposx).w,d1
		neg.w	d1
		move.w	#$DF,d2
		moveq	#0,d4
...
Deform_SecretBoss:
	...
	.setbottom:
		subq.w	#1,d2
		bmi.s	.end
		move.w	(v_bgscreenposx).w,d3

	.setbottomloop:
		move.w	d0,(a1)+
		move.w	d3,(a1)+
		dbf	d2,.setbottomloop
...
SH_BehindMid:
		cmpi.w	#-16,d0		; is Sonic within 16px of middle area?
		bge.s	SH_Behind16	; if yes, branch
		move.w	#-16,d0		; set to 16 if greater
